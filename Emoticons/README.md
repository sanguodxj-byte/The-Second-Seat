...
欢迎使用 AI 叙事者表情包功能！叙事者可以根据对话内容自动选择表情包来表达情感。
...
### 尺寸建议
- **推荐尺寸**: 128x128 至 512x512 像素
- **最小尺寸**: 64x64 像素
- **最大尺寸**: 1024x1024 像素（过大会影响性能）
...
### 为什么需要元数据？
元数据文件用于给表情包添加**情感标签**，帮助 AI 在对话时自动选择合适的表情包。
...
### 2. 情绪状态
- 当前情绪是"喜悦"：选择 `happy`, `joy`
- 当前情绪是"愤怒"：选择 `angry`, `frustrated`
- 当前情绪是"忧郁"：选择 `sad`, `disappointed`

### 3. 对话内容关键词
- 包含"哈哈"、"太好了"：选择 `happy`, `excited`
- 包含"天哪"、"什么"：选择 `surprised`
- 包含"..."、"嗯"：选择 `thinking`
- 包含"唉"、"遗憾"：选择 `sad`
...
- **Photopea**: https://photopea.com (免费PS替代)
...
### Q: 表情包会影响游戏性能吗？
A: 不会。表情包只在对话时加载和显示，数量控制在合理范围内（<50个）不会影响性能。

### Q: 可以使用动画表情包吗？
A: 目前只支持静态图片（PNG/JPG），不支持 GIF 动画。

### Q: AI 一定会使用表情包吗？
A: 不一定。AI 会根据对话内容和情感判断是否需要表情包。你也可以在对话中明确要求"用表情包回复我"。

### Q: 可以禁用表情包功能吗？
A: 可以。删除 `Emoticons/` 文件夹内的所有图片即可禁用。
